...
# Copied from the container with
# $ MSYS_NO_PATHCONV=1 docker compose exec web cat /var/www/mediawiki/w/extensions/SemanticMediaWiki/maintenance/updateQueryDependencies.php > updateQueryDependencies.php
# and then modified to use the job queue, MBSD-387
...
class updateQueryDependencies extends \Maintenance {
	...
	private function runUpdate() {
		...
		// WIKITEQ PATCH: change message from "Performing" to "Enqueueing"
		$this->reportMessage(
		...
		// WIKITEQ PATCH: add $jobs variable
		$jobs = [];
		foreach ( $res as $row ) {
			$i++;
			...
			// WIKITEQ PATCH: replace `$updateJob->run();` with adding to array
			$jobs[] = $updateJob;
		}
		// WIKITEQ PATCH: push jobs
		$applicationFactory->getJobQueue()->push( $jobs );
